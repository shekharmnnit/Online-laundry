...
$email=$_SESSION['email'];
 $query = "SELECT * FROM tbl_order_items WHERE status='$status' ORDER BY $orderby desc";  
...
<!-- header -->
  ...
  <!--//header-w3l-->
          ...
          <div style="float:left;color:red; background-color:#BDB76B;width:20%;height:800px;"> 
          <a href="admin_history.php?update=<?php echo "not_receaved"; ?>" class="edit_btn" >new</a><br>
          <a href="admin_history.php?update=<?php echo "receaved"; ?>" class="edit_btn" >receaved</a><br>
          <a href="admin_history.php?update=<?php echo "washed"; ?>" class="edit_btn" >washed</a><br>
          <a href="admin_history.php?update=<?php echo "delivered"; ?>" class="edit_btn" >delivered</a><br><br><br>

          <h5>sortby</h5>
          <a href="admin_history.php?sortby=<?php echo "presentdate" . " update= " . "$status";?>" class="edit_btn" >presentdate</a><br>
          <a href="admin_history.php?sortby=<?php echo "item_name" . " update= " . "$status"; ?>" class="edit_btn" >deliverydate</a><br>
          <a href="admin_history.php?sortby=<?php echo "item_quantity" . " update= " . "$status"; ?>" class="edit_btn" >price</a><br>
          <a href="admin_history.php?sortby=<?php echo "email" . " update= " . "$status"; ?>" class="edit_btn" >email</a><br>
          </div>
           ...
           <div class="container" style="padding-left: 20%">  
                <h2 align="center">admin_history</h2>  <br>
                ...
                <div id="order_table">  
                     ...
                     ?>  
                          <tr>  
                               ...
                               <td><table class="table table-bordered" style="background-color: #DEB887;"><tr style="background-color:  #FF7F50;"><td><?php echo $row["email"]; ?></td><td><?php echo $row["presentdate"]; ?></td><td><?php echo $row["item_name"]; ?></td></tr><tr><th>item</th><th>unit</th><th>price</th></tr> <?php for( $i=0; $i<count($myArray)-2; $i=$i+3 ){echo '<tr><td>'.$myArray[$i].'</td>'.'<td>'. $myArray[$i+1] .'</td>' . '<td>'. $myArray[$i+2] .'</td>' .'</tr>';} ?>
                                 <tr>
                                  <th></th>
                                  <th>delivery</th>
                                 ...
                                 <tr>
                                  <th></th>
                                  <th>total</th>
                                   <th><?php echo $myArray[$i+1];$total=$total+$myArray[$i+1];?></th>
                                 </tr>
                               </table></td>
                               <?php if ($status=="not_receaved"): ?>
                               <td><a href="status.php?edit=<?php echo $row['order_id']; ?>" class="edit_btn" >receaved</a></td>  
                               <?php elseif($status=="receaved"):?>
                               <td><a href="status.php?update=<?php echo $row['order_id']; ?>" class="edit_btn" >washed</a></td>
                               <?php elseif($status=="washed"):?>
                               <td><a href="status.php?del=<?php echo $row['order_id']; ?>" class="edit_btn" >delivered</a></td>
                               <?php endif ?>
                          </tr>  
                     <?php  
                     }  
                     ...
                     <div style="float:left;"><?php echo "total price=" ." ".$total?></div>  
                </div>  
           </div>         
         </header>
